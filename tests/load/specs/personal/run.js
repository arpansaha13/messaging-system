...
const MESSAGING_MIGRATIONS_DIR = '../../apps/backend/server/migrations'
...
try {
  ...
  const result = spawnSync('k6', ['run', 'specs/personal/personal.k6.js'], { stdio: 'inherit' })
  exitCode = result.status ?? 1
} catch (error) {
  console.error('[load:personal] Failed to seed or run k6:', error)
  exitCode = 1
} finally {
  if (seeded) {
    try {
      ...
      console.error('[load:personal] Cleanup failed:', error)
      exitCode = 1
    }
  }
}
