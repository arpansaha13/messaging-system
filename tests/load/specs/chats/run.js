...
try {
  ...
  const result = spawnSync('k6', ['run', 'specs/chats/chats.k6.js'], { stdio: 'inherit' })
  exitCode = result.status ?? 1
} catch (error) {
  console.error('[load:chats] Failed to seed or run k6:', error)
  exitCode = 1
} finally {
  if (seeded) {
    try {
      ...
      console.error('[load:chats] Cleanup failed:', error)
      exitCode = 1
    }
  }
}
