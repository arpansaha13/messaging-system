...
func TestValidator_ValidateEmail(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				assert.Error(t, err)
				assert.IsType(t, &gtk.ValidationError{}, err)
				assert.Equal(t, "email", err.(*gtk.ValidationError).Field)
			} else {
...
func TestValidator_ValidateUsername(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				assert.Error(t, err)
				assert.Equal(t, "username", err.(*gtk.ValidationError).Field)
			} else {
				assert.NoError(t, err)
			}
		})
...
func TestValidator_ValidatePassword(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				assert.Error(t, err)
				assert.Equal(t, "password", err.(*gtk.ValidationError).Field)
			} else {
				assert.NoError(t, err)
			}
		})
