import type { ChatBodyProps } from './ChatBody'
import type { ChatHeaderProps } from './ChatHeader'
import type { ChatFooterProps } from './ChatFooter'

interface ChatProps {
  header: React.ReactElement<ChatHeaderProps>
  body: React.ReactElement<ChatBodyProps>
  footer: React.ReactElement<ChatFooterProps>
}

export default function Chat(props: Readonly<ChatProps>) {
  const { header, body, footer } = props

  return (
    <div className="flex h-full flex-col">
      <div className="relative z-10 flex items-center justify-between bg-gray-100 px-4 py-2.5 shadow-sm shadow-gray-400/30 dark:bg-gray-800 dark:shadow-none">
        {header}
      </div>

      <div className="flex flex-grow flex-col justify-end overflow-hidden bg-slate-200/50 dark:bg-gray-900">{body}</div>

      <div className="relative z-10 flex w-full items-center space-x-1 bg-gray-100 px-4 py-2.5 text-gray-900 shadow-[0_-1px_2px_0] shadow-gray-300/20 dark:bg-gray-800 dark:text-gray-400 dark:shadow-none">
        {footer}
      </div>
    </div>
  )
}
